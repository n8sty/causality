...
class TestDID(TestAPI):
    def setUp(self):
        ...
        start = assignment + np.random.normal(1, size=SIZE)
            ...
            start + np.random.normal(2.0 * assignment) + np.random.normal(2, size=SIZE)
        )
        self.X_pre = pd.DataFrame(
            {"Start": pre_experiment, "End": start, "assignment": assignment}
        )
...
class TestPropScore(TestAPI):
    def test_match(self):
        ...
        X = pd.DataFrame(
            {"assignment": [1, 0, 0, 0, 0, 0], "propensity score": [3, 1, 2, 3, 5, 4]}
        )
    ...
    def test_score(self):
        ...
        p_assign = np.exp(z1 + z2_numeric) / (1.0 + np.exp(z1 + z2_numeric))
        assignment = np.random.binomial(1, p_assign)
        outcome = np.random.normal(assignment)
        matcher = PropensityScoreMatching()
        X = pd.DataFrame(
            {"z1": z1, "z2": z2, "assignment": assignment, "outcome": outcome}
        )
    ...
    def test_at_estimators(self):
        ...
        y = (
            np.random.normal(size=N) + (z1 + z2 + z3 + 1.0) * d
...
class TestIPW(TestAPI):
    def test_estimators(self):
        ...
        d = np.random.binomial(1, p=1.0 / (1.0 + np.exp(-z)))
        ...
        atc_lower, atc_exp, atc_upper = ipw_model.estimate_ATC(
            X,
            assignment,
            outcome,
            confounder_types,
        ...
        assert (
            0.9 * atc_lower
            <= (X[X["d"] == 0]["y1"] - X[X["d"] == 0]["y0"]).mean()
            <= 1.1 * atc_upper
        )

        att_lower, att_exp, att_upper = ipw_model.estimate_ATT(
            X,
            assignment,
            outcome,
            confounder_types,
        ...
        assert (
            0.9 * att_lower
            <= (X[X["d"] == 1]["y1"] - X[X["d"] == 1]["y0"]).mean()
            <= 1.1 * att_upper
        )

        ate_lower, ate_exp, ate_upper = ipw_model.estimate_ATE(
            X,
            assignment,
            outcome,
            confounder_types,
