...
class CausalDataFrame(pd.DataFrame):
    def zmean(self, *args, **kwargs):
        if kwargs.get("confidence_level", None) and not kwargs.get(
            "bootstrap_samples", 0
        ):
            kwargs["bootstrap_samples"] = 500
        if not kwargs.get("bootstrap_samples", 0) and not kwargs.get(
        ...
        uppers = []
        expecteds = []
        z = ss.norm().ppf(1.0 - (1.0 - confidence_level) / 2.0)
        for xi in unique_x:
            kwargs["xi"] = xi
    ...
    def _line_zplot(self, *args, **kwargs):
        ...
        xs = []
        ys = []
        xmin, xmax = kwargs.get(
            "xlim", (self[treatment].quantile(0.01), self[treatment].quantile(0.99))
        )
        for xi in np.arange(xmin, xmax, (xmax - xmin) / 100.0):
            df = self.copy()
            df[treatment] = xi
            df["$E[Y|X=x,Z]$"] = model.predict(df[[treatment] + confounders])
            yi = df.mean()["$E[Y|X=x,Z]$"]
    ...
    def _bootstrapped_mean_zplot(self, *args, **kwargs):
        ...
        kwargs["kind"] = "bar"
        if kwargs.get("bootstrap_samples", 0) > 1 or kwargs.get(
            "confidence_level", None
        ):
            df["{}_lower".format(kwargs["y"])] = (
                df[kwargs["y"]] - df["{}_lower".format(kwargs["y"])]
            )
            df["{}_upper".format(kwargs["y"])] = (
                df["{}_upper".format(kwargs["y"])] - df[kwargs["y"]]
            )
            kwargs["yerr"] = df[
                ["{}_lower".format(kwargs["y"]), "{}_upper".format(kwargs["y"])]
            ].values.T
        for k in ["xi", "z", "bootstrap_samples", "z_types", "confidence_level"]:
            if k in kwargs:
                del kwargs[k]
...
class KernelModelWrapper(object):
    ...
    def predict(self, X):
        if X.shape != self.X_shape:
            raise Exception(
                "Expected shape {}, received {}".format(self.X_shape, X.shape)
            )
