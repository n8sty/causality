...
class CausalEffect(object):
    ...
    def __get_support(self, X):
        ...
        """
        data_support = {
            variable: (X[variable].min(), X[variable].max()) for variable in X.columns
        }
        variable_bandwidths = {
            variable: bw
            for variable, bw in zip(
                self.effects + self.conditional_density_vars,
        ...
        for variable in self.effects + self.conditional_density_vars:
            if self.variable_types[variable] == "c":
                lower_support = (
                    data_support[variable][0] - 10.0 * variable_bandwidths[variable]
                )
                upper_support = (
                    data_support[variable][1] + 10.0 * variable_bandwidths[variable]
                )
        ...
        data = pd.DataFrame(
            {
                k: [v]
                for k, v in zip(
                    self.continuous_Z + self.discrete_Z + self.causes + self.effects,
                    args,
                )
            }
        )
    ...
    def expectation_integration_function(self, *args):
        data = pd.DataFrame(
            {
                k: [v]
                for k, v in zip(self.continuous_Z + self.discrete_Z + self.causes, args)
            }
        )
        conditional = self.conditional_expectation.fit(
            data_predict=data[self.conditional_density_vars].values
        )[0]
        density = self.density.pdf(data_predict=data[self.admissable_set])
        return conditional * density
        ...
        if self.discrete_Z:
            ...
            for z_vals in itertools.product(*discrete_variable_ranges):
                z_discrete = pd.DataFrame(
                    {k: [v] for k, v in zip(self.discrete_Z, z_vals)}
                )
                if self.continuous_Z:
                    continuous_Z_ranges = [
                        self.support[variable] for variable in self.continuous_Z
                    ]
                    args = z_discrete.join(x).values[0]
                    causal_effect += nquad(
                        self.integration_function, continuous_Z_ranges, args=args
                    )[0]
                else:
                    z_discrete = z_discrete[self.admissable_set]
                    exog_predictors = x.join(z_discrete)[self.conditional_density_vars]
                    conditional = self.conditional_density.pdf(
                        exog_predict=exog_predictors, endog_predict=x[self.effects]
                    )
            ...
            causal_effect, error = nquad(
                self.integration_function, continuous_Z_ranges, args=tuple(x.values[0])
            )
            return causal_effect
        else:
            return self.conditional_density.pdf(
                exog_predict=x[self.causes], endog_predict=x[self.effects]
            )
