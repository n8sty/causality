...
class ChiSquaredTest:
    def __init__(self, y, x, z, data, alpha):
        ...
        for xi, yi in itertools.product(x, y):
            ...
            tables = (
                tables.join(pd.get_dummies(data[yi], prefix=yi))
                .groupby(groupby_key)
                .sum()
            )
            del tables[yi]
            ...
            for contingency in contingencies:
                ...
                try:
                    ...
                    raise Exception(
                        "Not enough data or entries with 0 present: Chi^2 Test not applicable."
                    )
                self.total_dof += dof
                self.total_chi2 += chi2
        self.total_p = 1.0 - scipy.stats.chi2.cdf(self.total_chi2, self.total_dof)
...
class MutualInformationTest:
    ...
    def __init__(self, y, x, z, X, alpha, variable_types={}):
        ...
        z = scipy.stats.norm.ppf(1.0 - alpha / 2.0)  # one-sided
        self.dI = z * self.dI

    def independent(self):
        if self.I - self.dI > 0.0:
            return False
        else:
            return True
    ...
    def discrete_mutual_information(self, x, y, z, X):
        ...
        for zi, n_zi in n_z.items():
            ...
            conditional_informations[zi] = self.max_likelihood_information(
                x, y, zi_subset
            )
        I_ml = sum(
            [(kz / N) * conditional_informations[zi][0] for zi, kz in n_z.items()]
        )
        dI_ml = np.sqrt(
            sum(
                [
                    ((kz / N) * conditional_informations[zi][1]) ** 2.0
                    for zi, kz in n_z.items()
                ]
            )
        )
        return I_ml, dI_ml
    ...
    def max_likelihood_information(self, x, y, X):
        ...
        for xy in X[x + y].values:
            ...
            n_x[tuple(xi)] += 1.0
            n_y[tuple(yi)] += 1.0
            n_xy[(tuple(xi), tuple(yi))] += 1.0
        N = sum(n_x.values())
        I_ml = sum(
            [
                (k / N) * np.log(k * N / float(n_x[xi] * n_y[yi]))
                for (xi, yi), k in n_xy.items()
            ]
        )
        K = sum(
            [
                (k / N) * (np.log(k * N / float(n_x[xi] * n_y[yi]))) ** 2.0
                for (xi, yi), k in n_xy.items()
            ]
        )
        return I_ml, np.sqrt((K - I_ml ** 2.0) / (N + 1.0))
